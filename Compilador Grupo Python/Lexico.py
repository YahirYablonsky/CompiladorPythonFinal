...
def estaEnTablaSimbolos():
    for elemento in tablaDeSimbolos:
...
def t_FLOATD(t):
    ...
    if( t.value >(3.4028237*(10^38))  or t.value <(1.175494*(10^-38)) ):
        ...
        return None   
    valor = "_"+str(t.value)
    if (valor not in getNombreTablaSimbolos()):
        tablaDeSimbolos.append(TablaDeSimbolos(valor,"FLOAT",t.value,"","",""))       
    return t 
...
def t_INTEGER(t):
    ...
    valor = "_"+str(t.value)
    if (valor not in getNombreTablaSimbolos()):
        tablaDeSimbolos.append(TablaDeSimbolos(valor,"INT",t.value,"","",""))           
    return t
...
def t_STRINGD(t):
    ...
    if (valor not in getNombreTablaSimbolos()):
        tablaDeSimbolos.append(TablaDeSimbolos("_"+valor.replace(" ","_"),"STRING",'\"'+valor+'\"',"",30,len(valor)))    
    return t
